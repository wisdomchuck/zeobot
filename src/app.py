...
bot = commands.Bot(command_prefix=config.command_prefix, description="A multi-purpose Ruby Rose from RWBY themed discord bot", pm_help=True, intents=intents)
...
async def main():
    ...
    async with aiohttp.ClientSession() as our_client:
        # 2. We become responsible for starting the bot.

        exts = ['general', 'mod', 'dice']
        async with CustomBot(commands.when_mentioned, web_client=our_client, initial_extensions=exts) as bot:
